# 👋 Hi, I'm Ishtiaque Ahmed Tanim

💡 Junior Software Quality Assurance (SQA) Engineer with experience in manual, API, web, mobile, and automation testing. Passionate about delivering stable, scalable, and high-quality software products through effective testing strategies.

💼 Currently working as a Junior SQA Engineer at AKIJ iBOS Limited

---

## 🔍 What I Do

- 🧪 Design and execute test cases, test plans, and test scenarios to ensure software quality
- 🐞 Identify, document, and track bugs throughout the software development lifecycle
- 🤖 Build automation frameworks using Selenium & Playwright
- 📱 Perform mobile automation testing using Appium & Maestro
- 🔗 Conduct API testing using Postman
- ⚡ Perform performance and load testing using JMeter
- 📊 Analyze application stability, usability, and performance under different conditions

---

## 🛠 Tech Stack & Tools

### 🚀 Automation Testing
![Selenium](https://img.shields.io/badge/Selenium-green?style=for-the-badge&logo=selenium)
![Playwright](https://img.shields.io/badge/Playwright-black?style=for-the-badge&logo=playwright)
![Appium](https://img.shields.io/badge/Appium-purple?style=for-the-badge&logo=appium)
![Maestro](https://img.shields.io/badge/Maestro-blue?style=for-the-badge)

### 🔗 API Testing
![Postman](https://img.shields.io/badge/Postman-orange?style=for-the-badge&logo=postman)

### ⚡ Performance Testing
![JMeter](https://img.shields.io/badge/JMeter-red?style=for-the-badge)

### 💻 Languages
![JavaScript](https://img.shields.io/badge/JavaScript-yellow?style=for-the-badge&logo=javascript)
![Python](https://img.shields.io/badge/Python-blue?style=for-the-badge&logo=python)

### ⚙️ CI/CD & Version Control
![Git](https://img.shields.io/badge/Git-black?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-black?style=for-the-badge&logo=github)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-black?style=for-the-badge&logo=githubactions)

### 🧰 Tools
![Jira](https://img.shields.io/badge/Jira-blue?style=for-the-badge&logo=jira)
![XMind](https://img.shields.io/badge/XMind-orange?style=for-the-badge)

---

## 🚀 Featured Projects

### 🔹 Playwright Automation Framework
- Automated end-to-end testing using Playwright
- Implemented Page Object Model (POM)
- Added reusable utilities and reporting

### 🔹 Mobile Automation Testing
- Built Android automation flows using Appium & Maestro
- Automated login, navigation, and validation scenarios

### 🔹 API Testing Collection
- Created API test collections using Postman
- Performed request validation and response verification

### 🔹 Performance Testing with JMeter
- Conducted load and stress testing
- Analyzed response time and application stability

---

## 📌 Current Focus

- 🚀 Improving mobile automation frameworks using Appium & Maestro
- 📚 Learning advanced test architecture and scalable automation design
- ⚙️ Exploring CI/CD integration with automation testing
- 🧠 Expanding knowledge in performance and security testing

---
## 📫 Connect With Me

- 📧 Email: ishtiaqueahmed1998@gmail.com
- 💼 LinkedIn: https://www.linkedin.com/in/ishtiaque2/
- 🌐 Portfolio: [tanimqa](https://tanimqa.vercel.app/)

---

## ⚡ Fun Fact

📸 I enjoy photography and exploring new technologies in my free time.

---

⭐ *“Quality is never an accident; it is always the result of intelligent effort.”*
